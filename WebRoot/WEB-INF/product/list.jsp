...
<table style="margin-top:10px;" width="999" align="center" cellpadding="0" cellspacing="0" class="thin-border">
	...
	<td><a href="ProductServlet?method=show&id=${p.id }">${p.name }</a></td><td>${p.price }</td>
	<td>${p.stock }&nbsp;<c:if test="${not empty loginUser and loginUser.type eq 1 }"><a href="ProductServlet?method=addStockInput&id=${p.id }">添加</a></c:if></td>
	<td>
		...
		<c:if test="${not empty loginUser and loginUser.type eq 1 }">
			<a href="ProductServlet?method=changeStatus&id=${p.id }">变更</a>
		</c:if>
	</td>
	<td>
	<c:if test="${not empty loginUser and loginUser.type eq 1 }">
	<a href="ProductServlet?method=updateInput&id=${p.id }">修改</a> <a href="ProductServlet?method=delete&id=${p.id }">删除</a>
	</c:if>
	<c:if test="${p.status eq 1 }"><a href="OrdersServlet?method=addToCart&id=${p.id}">加入购物车</a></c:if>
	&nbsp;
	</td>
	</tr>
	</c:forEach>
	...
	<td colspan="6">
		<jsp:include page="/inc/pager.jsp">
			<jsp:param value="ProductServlet" name="url"/>

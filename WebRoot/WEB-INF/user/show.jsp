...
<table width="800" class="thin-border" align="center" class="thin-border" cellpadding="0" cellspacing="0">
	<tr>
	...
	<tr>
		<td>
			...
			<a href="AddressServlet?method=addInput&userId=${user.id }">添加联系地址</a>
		</c:if>
		</td>
	</tr>
	...
	<c:forEach items="${user.addresses}" var="address">
		<tr>
			<td>
			${address.name},${address.phone} ,${address.postcode }
			<a href="AddressServlet?method=updateInput&id=${address.id}">修改</a>
			<a href="AddressServlet?method=delete&id=${address.id}&userId=${user.id}">删除</a>
			</td>
		</tr>
	</c:forEach>
	</c:if>
